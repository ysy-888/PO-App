...
export function createPackingHtmlBuilder(ctx) {
  ...
  function buildPlPrintPackingListHtml(row) {
    ...
    const packingList = ctx.getPackingListForPo(poNumber);
    const cartons = plActiveCartons(ctx.getCartonsForPo(poNumber));
...
${notesHtml}
  ...
  function buildPlPrintPoSectionHtml(row, { showInternalPoFields = true, showExfDate = true } = {}) {
    ...
    const activeCartons = plActiveCartons(ctx.getCartonsForPo(String(row["PO #"] ?? "")));
...
<div class="pl-print-page">
  ...
  function plPrintCtnQtyForPo(row) {
    const cartons = plActiveCartons(ctx.getCartonsForPo(String(row["PO #"] ?? "")));
    return cartons.length || toQtyNumber(row["Ctn Qty"]);
  }

  function plPrintWeightForPo(row) {
    const cartons = ctx.getCartonsForPo(String(row["PO #"] ?? ""));
  ...
  function buildPlPrintTitlePageHtml(rows, {
    ...
    const headerRightPrimary = isAsnOrShipmentCover
      ...
      : plPrintEsc(label);
    const labelColspan = isAsnOrShipmentCover
      ? (showInternalPoFields ? 6 : 5)
      : ((isAsnTitlePage ? 6 : 5) - (showInternalPoFields ? 0 : 1));
    ...
    const coverTableHead = isAsnOrShipmentCover
      ? `<tr>
          ...
          <th class="pl-center">${showInternalPoFields ? "Buyer PO #" : "Buyer PO"}</th>
...
<div class="pl-print-page">
  ...
  <div class="pl-body">
    ...
    <table class="pl-carton-table pl-title-table" cellpadding="0" cellspacing="0">
      ${plPrintTitleColgroup({ showInternalPoFields, isAsnTitlePage, isAsnOrShipmentCover })}
      <thead>
        ${coverTableHead}
      </thead>
...
<style type="text/css">
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  return {
    buildGroupPackingListHtml(poNumbers, {
    ...
    buildCartonLabelsPrintHtml(poNumbers, labelInputs) {
      ...
      rows.forEach(row => {
        ...
        const cartons = plActiveCartons(ctx.getCartonsForPo(po)).slice().sort((a, b) =>
...
export function wrapPrintHtml(bodyHtml) {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Packing List</title></head><body>${bodyHtml}</body></html>`;
}
